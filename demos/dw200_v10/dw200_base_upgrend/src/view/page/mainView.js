...
mainView.init = function () {
    ...
    // Use more modern dark blue background
    ...
    mainView.netInfo.forEach((item, index) => {
        ...
        title.textColor(0x374151) // Dark gray text
        ...
        if (item.type === 'label') {
            ...
            label.textColor(0x6b7280) // Medium gray text
            item.label = label
        }
    ...
    try {
        if (std.exist('/app/code/src/config.json')) {
            ...
            log.info("[mainView] Read config file:", configData)
    ...
    mainView.netInfo[3].dropdown.on(dxui.Utils.EVENT.VALUE_CHANGED, () => {
        ...
        if (selectedDhcp === 1) { // yes (DHCP auto obtain)
            ipItem.hide()
            maskItem.hide()
            gatewayItem.hide()
            dnsItem.hide()
    ...
    if (currentDhcp === 1) { // yes (DHCP auto obtain)
        ipItem.hide()
        maskItem.hide()
        gatewayItem.hide()
        dnsItem.hide()
