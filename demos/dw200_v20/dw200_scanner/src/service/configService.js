...
const supported = {
    sysInfo: {
        ...
        //网络类型  1以太网 0关闭网络
        net_type: { rule: v => [0, 1, 2, 4].includes(v) },
        //mac地址 0默认 2手动设置
        ...
        reboot: { rule: () => true },
        // 设置系统时间，秒级
        time: { rule: regnCheck, callback: v =>setTime(v) },
...
// 需要重启的配置
const needReboot = ['owifi', 'w_mode', 'ntpLocaltime', 'codeSwitch', 'nfc', 'taddr', 'port', 'de_type',
    'net_type', 'ip_mode', 'ip', 'gateway', 'dns', 'mask', 'fixed_macaddr_enable', 'ntp_en', 'ntp_addr', 'ntp_port', 'ochannel', 'sysInfo.autoRestart']
