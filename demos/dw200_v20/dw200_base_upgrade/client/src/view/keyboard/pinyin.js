...
/**
 * Show keyboard, need to initialize first
...
// Lock keyboard, not allowed to switch modes
...
function createPinyin() {
    ...
    previewBox.fillData = (str) => {
        ...
        if (str.length > 8) {
            // If more than 8 characters, show more characters button
            morePreview.show()
        } else {
            morePreview.hide()
        }
    }
    // More Chinese character preview button
    ...
    rightBtn.source('/app/code/resource/image/key/right.png')
    ...
    phrase.text = (v) => {
        ...
        if (v.length > 10) {
            // Phrase preview length not more than 10 characters
            return
        }
        phrasePreview.show()
    ...
    pinyinPanel.show = () => {
        // Override show method, show Chinese character preview box
        previewBox.align(dxui.Utils.ALIGN.TOP_LEFT, 0, -70)
        morePreview.align(dxui.Utils.ALIGN.TOP_RIGHT, 0, -70)
        phrasePreview.align(dxui.Utils.ALIGN.TOP_LEFT, 0, -105)
    ...
    pinyinPanel.hide = () => {
        // Override hide method, hide Chinese character preview box
    ...
    // Look up dictionary, search based on input content
    function search() {
        // Input pinyin
        ...
        previewBox.fillData(characters)
            ...
            switch (id) {
                case 29:
                    // Backspace, delete phrase first if exists
