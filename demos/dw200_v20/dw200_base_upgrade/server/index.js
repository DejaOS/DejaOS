...
mqttClient.on('message', (topic, message) => {
    try {
        ...
        switch (topic) {
            case mqttConfig.topicPatterns.heartbeat:
                handleHeartbeatMessage(messageData);
                break;

            case mqttConfig.topicPatterns.offline:
                handleOfflineMessage(messageData);
                break;

            case mqttConfig.topicPatterns.upgradeReply:
...
io.on('connection', (socket) => {
    ...
    socket.on('sendUpgradeCommand', (data) => {
        ...
        const upgradeTopic = mqttConfig.topicPatterns.upgradeCommand.replace('{uuid}', uuid);
...
server.listen(PORT, () => {
    ...
    console.log('MQTT topics subscribed:', mqttConfig.topics);
});
