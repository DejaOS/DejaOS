...
import fingerEnterView from './view/config/menu/localUser/fingerEnterView.js'
import fingerApplyView from './view/config/menu/localUser/fingerApplyView.js'
...
screen.init = function () {
    ...
    if(driver.device.finger){
        fingerEnterView.init()
        fingerApplyView.init()
    }
...
screen.initLanguage = function () {
    try {
        const language = std.loadFile('/etc/app/region.conf')?.trim() || std.loadFile('/etc/app/.region')?.trim();
        if (language === "INTL") {
            if (!config.get('base.language') || config.get('base.language') == "CN") {
                config.setAndSave('base.language', "EN")
            }
        } else {
            config.setAndSave('base.language', "CN");
        }
    } catch (error) {
        config.setAndSave('base.language', "CN");
    }
...
screen.deleteUser = function (user) {
    ...
    if (vouchers.length > 0) {
        vouchers.forEach(element => {
            ...
            if(element.type == "500") {
                log.info("deleteUser删除指纹凭证: ", parseInt(element.code))
                driver.finger.delete(parseInt(element.code))
            }
        });
    }
...
screen.updateUser = function (user) {
    ...
    if (user.face && user.feature) {
        ...
        if (findByuserIdAndType.length <= 0) {
            let ret = driver.face.addFea(user.userId, user.feature)
            ...
            //新增一个
            ret = sqliteService.d1_voucher.save({ keyId: std.genRandomStr(32), type: "300", code: src, userId: user.userId, extra: JSON.stringify({ faceType: 0 }) })
            if (ret != 0) {
                return false
            }
        } else {
    ...
    // 修改指纹凭证
    if (user.fingerId != null && user.fingerId != undefined) {
        log.info("updateUser 修改指纹凭证(code fingerId): ", parseInt(user.fingerId))

        //查询是否有指纹凭证有更新没有新增
        let countByuserIdAndType = sqliteService.d1_voucher.countByuserIdAndType(user.userId, "500");
        if (countByuserIdAndType > 0) {
            ret = sqliteService.d1_voucher.updatecodeByuserIdAndtype(user.userId, "500", parseInt(user.fingerId))
            if (ret != 0) {
                return false
            }
        } else {
            //新增一个
            ret = sqliteService.d1_voucher.save({ keyId: std.genRandomStr(32), type: "500", code: parseInt(user.fingerId), userId: user.userId })
            if (ret != 0) {
                return false
            }
        }
        ...
        // 没有内容去数据库表删除一下
        sqliteService.d1_voucher.deleteByuserIdAndtype(user.userId, "500")
    }
...
screen.insertUser = async function (user) {
    ...
    const saveVoucher = async (type, code, feature) => {
        ...
        if (type == "500") {
            let fingerData = sqliteService.d1_voucher.findByCodeAndType(code, "500");
            if (fingerData.length > 0 && fingerData[0].userId != user.userId) {
                //存在不能添加返回失败
                log.info("指纹重复");
                return "localUserAddView.failFingerRepeat"
            }
            log.info("insertUser新增指纹凭证(code fingerId): ", parseInt(code))
        }
    ...
    if (success === true && user.fingerId != null && !(success = await saveVoucher("500", user.fingerId)));
    if (success === true) {
...
screen.getVoucher = function (userId) {
    ...
    log.info("getVoucher 获取到的人员信息: ", JSON.stringify(person))
    ...
    let fingerId_voucher = sqliteService.d1_voucher.find({ userId: userId, type: "500" })[0] || undefined
    let idCard_voucher
    try {
        idCard_voucher = JSON.parse(person[0].extra).idCard
    } catch (error) {
    }
    // log.info("getVoucher 获取到的密码凭证信息: ", JSON.stringify(pwd_voucher))
    // log.info("getVoucher 获取到的卡凭证信息: ", JSON.stringify(card_voucher))
    // log.info("getVoucher 获取到的人脸凭证信息: ", JSON.stringify(face_voucher))
    // log.info("getVoucher 获取到的指纹凭证信息: ", JSON.stringify(fingerId_voucher))
    return {
        id: userId,
        ...
        fingerId: fingerId_voucher ? fingerId_voucher.code : undefined,
...
screen.getUserById = function(userId) {
    let person = sqliteService.d1_person.find({ userId: userId });
...
// 根据人员ID和凭证类型删除凭证
screen.delUserVoucher = function (userId, type) {
    sqliteService.d1_voucher.deleteByuserIdAndtype(userId, type)
}
...
screen.faceAuthResult = function (bool) {
    ...
    if (bool) {
        // 成功，进入设置菜单
        driver.audio.play(`/app/code/resource/wav/${language}/recg_s.wav`)
        dxui.loadMain(configView.screenMain)
    } else {
        driver.audio.play(`/app/code/resource/wav/${language}/recg_f.wav`)
        // 失败，报错
...
// 删除指定ID的指纹
screen.deleteFinger = function (fingerId) {
    // 删除指纹凭证
    return driver.finger.delete(fingerId)
}

// 远程录入指纹
screen.enrollFinger = function (payload) {
    screenManager.resetTimers()
    fingerApplyView.applyFinger(payload)
    dxui.loadMain(fingerApplyView.screenMain)
}

// 远程录入指纹指令回复
screen.fingerReply = function (data) {
    let payload = screen.mqttReply(data.serialNo, { fingerFeature: data.fingerChar }, data.code)
    screen.driver.mqtt.send(config.get("mqtt.prefix") + "access_device/v2/cmd/control_reply", JSON.stringify(payload))
}

// 缓存指纹特征
screen.cacheFingerChar = function (userId, fingerChar) {
    fingerService.setFingerChar({ userId: userId, fingerChar: fingerChar })
}

// 中断指纹采集
screen.interruptFinger = function () {
    screen.cacheFingerChar(fingerEnterView.userId, null)
    fingerEnterView.userId = null
    fingerApplyView.interruptFinger()
}

// 校验通行记录数量
screen.verifyPassRecords = function (num) {
    const passCount = sqliteService.d1_pass_record.count()
    const overCount = passCount - num
    if (overCount <= 0) {
        return
    }
    log.info(passCount,num)
    const oldRecords = sqliteService.d1_pass_record.findAllOrderBytimeStampAsc({ page: 0, size: overCount })
    if (!oldRecords || oldRecords.length === 0) {
        return
    }
    oldRecords.forEach((record) => {
        if (record.type == 300 && record.code) {
            dxos.systemBrief(`rm -rf ${record.code}`)
        }
        sqliteService.d1_pass_record.deleteByid(record.id)
    })
}

// 保存配置
screen.saveConfig = function (configAll) { 
    console.log('configAll', JSON.stringify(configAll));
...
function busEvents() {
    ...
    bus.on("showMessage", screen.showMessage)
    ...
    bus.on("resetTimers", screenManager.resetTimers)
    bus.on("enrollFinger", screen.enrollFinger)
    bus.on("interruptFinger", screen.interruptFinger)
    bus.on("verifyPassRecords", screen.verifyPassRecords)
}
screen.updateNetStatus = function (data) {
    ...
    if (data == "connected" && param) {
        ...
        networkSettingView.netInfo.status.label.dataI18n = "networkSettingView.networkConnected"
        ...
        topView.mqttConnectState(false)
        networkSettingView.netInfo.status.label.dataI18n = "networkSettingView.networkUnconnected"
        mainView.ipLbl.text(" ")
        wechatBindView.wechatIpLbl.text(" ")
        wechatNetView.wechatIpLbl.text(" ")
    }
    i18n.refreshObj(networkSettingView.netInfo.status.label)
    if (screen.screenNow && screen.screenNow.id === "networkSettingView") {
...
screen.getFeaByCapEnd = function (data) {
    ...
    let language=config.get("base.language")||"CN";
    driver.audio.play(`/app/code/resource/wav/${language}/recognition_s.wav`)
...
// 展示成功/失败信息
screen.showMessage = function (data) {
    let { status, msg } = data
    if (status) {
        mainView.messagePanel.success(msg)
    } else {
        mainView.messagePanel.fail(msg)
    }
}
...
screen.changeLanguage = function () {
    let language = screen.getConfig()['base.language']
    i18n.setLanguage(language)
    driver.screen.reload(language)
}
