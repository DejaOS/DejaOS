    ...
    try {
        ...
        if (similarity === false) {
            ...
            if (!ret && config.get('mqtt.onlinecheck') == 1 && mqtt_map.get("MQTT_STATUS") == "connected") {
                ...
                driver.mqtt.send(config.get("mqtt.prefix") + "access_device/v2/event/access_online", JSON.stringify(mqttService.mqttReply(serialNo, data, mqttService.CODE.S_000)))
                driver.audio.play(`/app/code/resource/wav/${language}/verify.wav`)
        ...
        if (ret == true) {
            ...
            driver.audio.play(`/app/code/resource/wav/${language}/verify_s.wav`)
            driver.gpio.open()
            ...
            if (similarity !== false) {
                driver.audio.play(`/app/code/resource/wav/${language}/verify_f.wav`)
            } 
...
function reply(data, result) {
    ...
    if (record.type == 300) {
        if (config.get('access.uploadToCloud')) {
    ...
    driver.mqtt.send(config.get("mqtt.prefix") + "access_device/v2/event/access", JSON.stringify(payload))
}
