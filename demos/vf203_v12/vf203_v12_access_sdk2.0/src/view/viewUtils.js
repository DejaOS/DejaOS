...
viewUtils.font = function (size, style) {
    ...
    if (arr.length > 0) {
        ...
        let path = "/app/code/resource/font/AlibabaPuHuiTi-3-65-Medium.ttf"
        let language = config.get("base.language")
        if (language == "AR") {
            path = "/app/code/resource/font/Arabic.ttf"
        } else if (language == "KO") {
            path = "/app/code/resource/font/Korean.ttf"
        }

        const font = dxui.Font.build(path, size, style)
        fonts.push({
            size, style, font
        })
        return font
...
viewUtils.input = function (parent, id, mode, enter = () => { }, dataI18n) {
    ...
    input.on(dxui.Utils.EVENT.CLICK, () => {
        ...
        pinyin.show(currentMode !== undefined ? currentMode : pinyin.getMode(), (data) => {
    ...
    input.setKeyboardMode = function (newMode) {
        if ([0, 1, 2, 3].includes(newMode)) {
            currentMode = newMode;
        } 
    }
...
viewUtils.statusPanel = function (parent, successI18n, failI18n, cardArea) {
...
viewUtils.messagePanel = function (parent) {
    const successBg = dxui.Image.build(parent.id + 'successMsgBg', parent)
    successBg.source(screen.resourcePath.imagePath + '/successBg.png')
    successBg.align(dxui.Utils.ALIGN.BOTTOM_MID, 0, 0)
    successBg.hide()

    const successIcon = dxui.Image.build(parent.id + 'successMsgIcon', successBg)
    successIcon.source(screen.resourcePath.imagePath + '/success_fill.png')
    successIcon.align(dxui.Utils.ALIGN.CENTER, 0, 0)

    const successLbl = dxui.Label.build(parent.id + 'successMsgLbl', successBg)
    successLbl.textFont(viewUtils.font(38))
    successLbl.textColor(0xffffff)
    successLbl.dataI18n = ""
    successLbl.align(dxui.Utils.ALIGN.TOP_MID, 0, screen.screenSize.height * (188 / 1024))
    ...
    const failBg = dxui.Image.build(parent.id + 'failMsgBg', parent)
    failBg.source(screen.resourcePath.imagePath + '/failBg.png')
    failBg.align(dxui.Utils.ALIGN.BOTTOM_MID, 0, 0)
    failBg.hide()

    const failIcon = dxui.Image.build(parent.id + 'failMsgIcon', failBg)
    failIcon.source(screen.resourcePath.imagePath + '/delete_fill.png')
    failIcon.align(dxui.Utils.ALIGN.CENTER, 0, 0)

    const failLbl = dxui.Label.build(parent.id + 'failMsgLbl', failBg)
    failLbl.textFont(viewUtils.font(38))
    failLbl.textColor(0xffffff)
    failLbl.dataI18n = ""
    failLbl.align(dxui.Utils.ALIGN.TOP_MID, 0, screen.screenSize.height * (188 / 1024))
