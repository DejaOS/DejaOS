...
mainView.init = function () {
    ...
    v.chain(screenMain)
        ...
        .load(() => {
            ...
            if (screen.model == 'vf202' && screen.getConfig()["base.appMode"] == 1) {
                cardArea.show()
            } else {
                cardArea.hide()
            }
    ...
    // 刷卡区域提示
    const cardArea = v.create(screenMain, "Image")
    mainView.cardArea = cardArea
    v.chain(cardArea)
        .moveForeground()
        .source(screen.resourcePath.imagePath + 'tip_card.png')
        .bottom_mid(0, screen.screenSize.height * (-210 / 1024))
        .hide()
    ...
    mainView.statusPanel = v.statusPanel(screenMain, 'mainView.success', 'mainView.fail', cardArea)

    mainView.messagePanel = v.messagePanel(screenMain)
}
