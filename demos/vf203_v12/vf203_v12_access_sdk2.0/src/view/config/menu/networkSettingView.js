...
networkSettingView.init = function () {
    ...
    screenMain.on(dxui.Utils.ENUM.LV_EVENT_SCREEN_LOADED, () => {
        ...
        networkSettingView.netInfo.type.value = netList
        networkSettingView.netInfo.type.dropdown.setOptions(netList)
    ...
    Object.entries(networkSettingView.netInfo).forEach(([key, item]) => {
        ...
        if (item.type === 'dropdown') {
            ...
            if (key === 'type') {
                dropdown.on(dxui.Utils.EVENT.VALUE_CHANGED, () => {
                    // 获取当前保存的网络类型
                    let currentType = screen.getConfig()["net.type"]
                    // 计算下拉框选中的新类型
                    let newType;
                    switch (dropdown.getSelected()) {
                        case 0: newType = 1; break;
                        case 1: newType = (screen.dxDriver.DRIVER.MODEL == "vf202") ? 4 : 2; break;
                        case 2: newType = 4; break;
                        default: return;
                    }
                    // 如果类型未发生变化，不做任何操作
                    ...
                    // 类型已变化，执行断开和后续切换逻辑
                    screen.driver.net.disconnect()
                    switch (dropdown.getSelected()) {
                        case 0:
                            networkSettingView.changeNetType(1)
                            screen.saveConfig({ net: { type: 1 } })
                            // 以太网
                            break;
                        case 1:
                            if (screen.dxDriver.DRIVER.MODEL == "vf202") {
                                screen.saveConfig({ net: { type: 4 } })
                                networkSettingView.changeNetType(4)
                            } else {
                                screen.saveConfig({ net: { type: 2 } })
    ...
    confirmBtn.on(dxui.Utils.EVENT.CLICK, () => {
        wifiListBoxClose.send(dxui.Utils.EVENT.CLICK)
        networkSettingView.netInfo.ssid.input.text(networkSettingView.selectedValue)
    })
    ...
    const saveBtn = viewUtils.bottomBtn(screenMain, screenMain.id + 'saveBtn', 'networkSettingView.save', () => {
        ...
        let netType = 1
        switch (networkSettingView.netInfo.type.dropdown.getSelected()) {
            case 0:
                netType = 1
                break;
            case 1:
        ...
        let saveConfigData = {
            "net": {
                "type": netType,
                "dhcp": networkSettingView.netInfo.dhcp.switch.isSelect() ? 2 : 1,
                "ip": networkSettingView.netInfo.ip.input.text(),
                "mask": networkSettingView.netInfo.mask.input.text(),
                "gateway": networkSettingView.netInfo.gateway.input.text(),
                "dns": networkSettingView.netInfo.dns.input.text(),
                "psk": networkSettingView.netInfo.psk.input.text(),
                "ssid": networkSettingView.netInfo.ssid.input.text()
            }
        }
        if (!networkSettingView.netInfo.dhcp.switch.isSelect()) {
            switch (networkSettingView.netInfo.type.dropdown.getSelected()) {
                case 0:
                    // 以太网
                    saveConfigData = {
...
networkSettingView._isConfigChanged = function () {
    ...
    const selectedTypeIndex = networkSettingView.netInfo.type.dropdown.getSelected()
    let currentType
...
networkSettingView.changeNetType = function (type) {
    if (type === 1) {
        // 以太网
        networkSettingView.netInfo.psk.obj.hide()
        networkSettingView.netInfo.ssid.obj.hide()
        networkSettingView.netInfo.status.obj.show()
        networkSettingView.netInfo.dhcp.obj.show()
        networkSettingView.netInfo.ip.obj.show()
        networkSettingView.netInfo.mask.obj.show()
        networkSettingView.netInfo.gateway.obj.show()
        networkSettingView.netInfo.dns.obj.show()
    } else if (type === 2) {
        // wifi
        networkSettingView.netInfo.psk.obj.show()
        networkSettingView.netInfo.ssid.obj.show()
        networkSettingView.netInfo.dhcp.obj.show()
        networkSettingView.netInfo.ip.obj.show()
        networkSettingView.netInfo.mask.obj.show()
        networkSettingView.netInfo.gateway.obj.show()
        ...
        networkSettingView.netInfo.psk.obj.hide()
        networkSettingView.netInfo.ssid.obj.hide()
        networkSettingView.netInfo.dhcp.obj.hide()
        networkSettingView.netInfo.ip.obj.hide()
        networkSettingView.netInfo.mask.obj.hide()
        networkSettingView.netInfo.gateway.obj.hide()
        networkSettingView.netInfo.dns.obj.hide()
    }
}
...
networkSettingView.refresh = function () {
    ...
    networkSettingView.netInfo.type.dropdown.setSelected(selectNum)
    networkSettingView.netInfo.ssid.input.text(configAll["net.ssid"])
    networkSettingView.netInfo.psk.input.text(configAll["net.psk"])
    networkSettingView.netInfo.dhcp.switch.select(configAll["net.dhcp"] == 2)
    networkSettingView.netInfo.ip.input.text(configAll["net.ip"])
    networkSettingView.netInfo.mask.input.text(configAll["net.mask"])
    networkSettingView.netInfo.gateway.input.text(configAll["net.gateway"])
    networkSettingView.netInfo.dns.input.text(configAll["net.dns"].split(",")[0] ? configAll["net.dns"].split(",")[0] : "")
    networkSettingView.netInfo.mac.label.text(configAll["net.mac"] ? configAll["net.mac"] : " ")
}
