...
pinyin.init = function (w, h) {
    ...
    pinyin.inited = true
    // 全局字体
    pinyin.font24 = dxui.Font.build('/app/code/resource/font/AlibabaPuHuiTi-3-65-Medium.ttf', 24, dxui.Utils.FONT_STYLE.NORMAL)
    ...
    function createKeyboard(capital) {
        ...
        englishKeyboard.obj.addEventCb((e) => {
            ...
            if (dsc.class_p == englishKeyboard.obj.ClassP && dsc.type == dxui.Utils.ENUM.LV_BTNMATRIX_DRAW_PART_BTN) {
                // 删除按钮图案添加
                if (dsc.id == 29) {
                    let src = screen.resourcePath.imagePath + '/backspace.png'
                    // 获取图片信息
                ...
                // 回车按钮图案添加
                if (dsc.id == 36) {
                    let src = screen.resourcePath.imagePath + '/enter.png'
                    // 获取图片信息
                    let header = dxui.Utils.GG.NativeDraw.lvImgDecoderGetInfo(src)
                    // 定义一块区域，居中显示，注意：尺寸转area需要-1，area转尺寸需要+1
                    let x1 = dsc.draw_area.x1 + (dsc.draw_area.x2 - dsc.draw_area.x1 + 1 - header.w) / 2;
                ...
                // 空格按钮图案添加
                if (dsc.id == 33) {
                    let src = screen.resourcePath.imagePath + '/space.png'
                    // 获取图片信息
                    let header = dxui.Utils.GG.NativeDraw.lvImgDecoderGetInfo(src)
                    // 定义一块区域，居中显示，注意：尺寸转area需要-1，area转尺寸需要+1
                    let x1 = dsc.draw_area.x1 + (dsc.draw_area.x2 - dsc.draw_area.x1 + 1 - header.w) / 2;
...
function createPinyin() {
    ...
    rightBtn.source(screen.resourcePath.imagePath + '/right.png')
