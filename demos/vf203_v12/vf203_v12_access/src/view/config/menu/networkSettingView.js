...
networkSettingView.init = function () {
    ...
    networkSettingBox.setSize(600, 700)
    ...
    networkSettingView.netInfo.forEach((item, index) => {
        ...
        networkSettingItem.setSize(560, 60)
        ...
        if (item.type === 'input') {
            const input = viewUtils.input(networkSettingItem, item.title + 'input', item.title == "networkSettingView.wifiPwd" ? undefined : 2, undefined, 'networkSettingView.input')
        ...
        if (item.type === 'dropdown') {
            ...
            if (item.title === 'networkSettingView.type') {
                dropdown.on(dxui.Utils.EVENT.VALUE_CHANGED, () => {
                    // screen.allowNetChange(false)
                    networkSettingView.changeNetType(dropdown.getSelected())
                    if (devType == 1 && dropdown.getSelected() == 1) {
                        // screen.switchNetworkType(dropdown.getSelected() + 3)
                    } else if (devType == 0 && dropdown.getSelected() == 1) {
    ...
    const saveBtn = viewUtils.bottomBtn(screenMain, screenMain.id + 'saveBtn', 'networkSettingView.save', () => {
        ...
        if (!networkSettingView.netInfo[3].switch.isSelect()) {
            ...
            } else if (devType == 0 && networkSettingView.netInfo[0].dropdown.getSelected() + 1 == 2) {
                // WIFI
                saveConfigData = {
                    "net": {
                        "type": networkSettingView.netInfo[0].dropdown.getSelected() + 1,
            ...
            } else if (devType == 1 && networkSettingView.netInfo[0].dropdown.getSelected() + 3 == 4) {
                //4G
                saveConfigData = {
                    "net": {
                        "type": networkSettingView.netInfo[0].dropdown.getSelected() + 3,
                    }
                }
            }
        } else {
