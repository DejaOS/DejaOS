    ...
    for (let i = 0; i < data.length; i++) {
        ...
        if (voucher.type == "300") {
            if (voucher.extra.faceType == 0) {
                ...
                if (res != 0) {
                    logger.info("注册人脸失败")
                    return faceService.regErrorEnum.feature[res + '']
                }
            }
        } else {
            record.code = voucher.code
...
mqttService.delKeyAgreement = function (data) {
    ...
    if (data.userIds && data.userIds.length > 0) {
        for (let i = 0; i < data.userIds.length; i++) {
            const element = data.userIds[i];
            // if (sqliteService.d1_voucher.countByUserId(element) == 0) {
            //     return "userId: " + element + " is not exist"
            // }
        }
...
mqttService.access_online_reply = function (data) {
    driver.mqtt.getOnlinecheckReply(JSON.parse(data.payload))
}
...
mqttService.report = function () {
    ...
    let payloadReply = mqttReply(std.genRandomStr(10), {
        ...
        releaseTime: config.get("sys.releaseTime"),
        netType: config.get("net.type")
    }, CODE.S_000)
...
// 接受一个字符串数组和最大长度参数，返回一个二维数组，每个子数组中字符串的总长度不超过指定的最大长度
function groupStrings(strings, maxLength = 40000) {
    const result = [];
    let currentGroup = [];
    let currentLength = 0;

    for (const str of strings) {
        // 如果添加当前字符串会超出长度限制，创建新分组
